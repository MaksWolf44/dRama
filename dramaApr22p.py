'''
The program creates what we call a drama plot. That is, it calculates as 2d histogram of rotational angles around alpha carbon.  

Data are taken from a text file generated by Gromacs. 

Results are saved in various formats from colour mapped plots to data in text file.  
'''
...
if __name__ == "__main__":
    ...
    parser.add_argument('--saveName',default='', help='File name for saved output')
...
def  drama(fileSys,fileRef, /, *,binSize = 3,saveDir = '',saveName='',borderline = None,vMax=None,saveAsTif = True, plotData = True, animatedGif=False, mixt=False,tfs=18,plotRaw=False,pureText=False):
    ...
    if pureText:                                   # pure text option disables all the plots reads ad saves obly text data
        saveAsTif = False
        animatedGif = False
        plotData = False
        plotRaw = False
    ...
    if saveAsTif:                                 
        import tifffile                             #  used save data to the text file
    ...
    if plotData or plotRaw:
        import matplotlib.pyplot as plt             # used to plot the data       
        import matplotlib.colors 
        print("ploting data")             
   
    if saveName == "":                              # if saveName not provided it saves the date under the name of of the first of the input file.
...
#-------------!!!!! Calculating differential Ramachndran - the esence of the script -------------------   
    ...
    normRefo = sum(sum(hir[0]))      # calculates number of data points. it is  needed for normalization                 # 
    hirn = np.flip(hir[0],0)/normRefo               # normalization      
    normSyso = sum(sum(his[0]))     # the same for reference data      
    ...
    absolut =  np.sum( np.abs(flatRama))    # 
...
#------------------------------------Saving results ----------------------------------------------------                                                                     
    ...
    if plotData:
        ...
        def plotPars():                                                 #setting plot params
            ...
            plt.xlabel(u'Φ [\u00B0]', fontsize = lfs)
            plt.ylabel(u'Ψ [\u00B0]', fontsize = lfs)
        ...
        plt.set_cmap(cmap)       
        plt.savefig(saveDirName+ "_diff_plot.png",bbox_inches='tight')
        if animatedGif:
            plt.plot()
            img_buf = io.BytesIO()
        ...
        cbar = plt.colorbar()
        cbar.ax.tick_params(labelsize=tfs) 
        plt.savefig(saveDirName+ "_ref_plot.png",bbox_inches='tight') 
        plt.clf()  
        ...
        cbar = plt.colorbar()
        cbar.ax.tick_params(labelsize=tfs) 
        plt.savefig(saveDirName + "_sys_plot.png",bbox_inches='tight') 
        plt.clf()       
    ...
    if animatedGif:                                                # saving as animated gif 
        ...
        giffSaveName = os.path.join(newDir,'animated.gif' )       
        tempName = os.path.join( newDir, 'animetedTemp.gif' )   
        ...
        if os.path.isfile(giffSaveName):                            # if animated file exists  
            ...
            imageObject.save(tempName, save_all=True, append_images = imagesToAppend[0:] , optimize=False, fps=2, loop=1) # and saved under temporary  name with a new frame  added            
            imageObject.close()
            ...
            print("animated gif added: " + str(giffSaveName))    
        
        else:  

            imagesToAppend[0].save(giffSaveName , append_images = imagesToAppend[0:], save_all=True, optimize=False, fps = 2, loop=1)    
            print("animated gif file created")
